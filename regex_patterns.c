...
// Will compile the regular expression and if no errors occurs
// return the compiled regular expression
regex_t get_comped_regex(struct regex_val reg){
    regex_t regex;
    char *regex_pattern = glob2regex(reg.glob);
    if (regcomp(&regex, regex_pattern, 0) == 0){
        return regex;
    }
    else{
        fprintf(stderr, "Error occured while doing regcomp()\n");
        exit(EXIT_FAILURE);
    }
}